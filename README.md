...
`ショートカットを作成.bat` をダブルクリックすると、**デスクトップとスタートメニューにアイコンが作られます**。
以降は普通のアプリと同じように、アイコンをダブルクリックするか、スタートメニューで「Voice Router」と検索して起動できます。

（作らずに `VoiceRouter.bat` から直接起動することもできます）
...
音声認識は一般的な語彙で学習されているため、研究分野の用語や社内用語、人名などは別の語に置き換わりがちです。`vocabulary.example.json` を `vocabulary.json` という名前でコピーし、対応表を書くと自動で直ります。
...
左が誤って認識される語、右が正しい表記です。長い語から順に置き換えます。
**アプリを再起動しなくても、ファイルを保存すれば次の発話から反映されます。**

> `vocabulary.json` は `.gitignore` に登録されており、**公開対象に含まれません**。
> 固有名詞や未公開の研究用語を書いても外部に出ることはありません。

日常語の一部に含まれる短い語を置換すると、関係のない箇所まで変わってしまいます。置換元はできるだけ具体的な語にしてください。
...
| `punctuate` | `true`（既定）で、認識結果に句点「。」を自動で補います |
| `input_device` | 使うマイク。`null`＝Windowsの既定。**`"ヘッドセット"` のように名前の一部**でも、番号でも指定できます |
...
| 認識精度が悪い（周囲がうるさい） | **ヘッドセットを使ってください。** ノートPC内蔵のマイクはスピーカーから出る音（動画・音楽）をそのまま拾うため、大きく精度が落ちます。`input_device` に `"ヘッドセット"` などを指定できます。イヤホンで音を出すだけでも改善します |
| 認識に句読点が付かない | `punctuate` が `true` か確認してください。読点「、」は付きません |
