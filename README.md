...
```
テンキー 1 → ChatGPT      テンキー 3 → ChatGPT (ブラウザ)
テンキー 2 → Claude       テンキー 4 → Gemini
```
...
1. 送信したいAIのアプリ（またはブラウザのタブ）を開いておく
2. 画面のボタンを**押しながら**話す
3. **離す** → 認識され、そのAIへ自動で入力・送信される

テンキーの **1〜4** でも同じ操作ができます（アプリが裏に隠れていても効きます）。
...
- 音声認識は CPU で動作します。GPUがなくても動きますが、長い発話ほど時間がかかります
