...
| 項目 | 説明 |
|------|------|
| `model_size` | `base`（最速・日本語の精度は落ちる） / `small`（既定・推奨） / `medium`（高精度・かなり重い） |
...
| 送信までが遅い | `model_size` を `base` にすると数倍速くなります（日本語の精度は落ちます）。`cpu_threads` を 4〜6 の範囲で調整するのも有効です |
...
- 音声認識は CPU で動作します。ボタンを離してから送信されるまで、既定の `small` モデルで **おおよそ3〜5秒**かかります（PCの性能と負荷によります）。急ぎたい場合は `model_size` を `base` にすると1〜2秒程度になりますが、日本語の精度は落ちます
- Whisper は音声を一定の長さに揃えて処理するため、**1秒の発話でも5秒の発話でも所要時間はあまり変わりません**。短く区切って何度も送るより、まとめて話す方が効率的です
